...
export interface VoiceCommand {
  ...
  patterns: string[];
  action: string;
  category: 'editor' | 'document' | 'project' | 'ai';
}
 ...
 * 检测文本中是否包含语音命令
 */
export function containsVoiceCommand(text: string): boolean {
 ...
 * 从识别的文本中处理语音命令
 */
export function processVoiceCommandFromTranscript(text: string): CommandDetectionResult {
  const normalizedText = text.toLowerCase().trim();
  ...
  // 提取剩余文本（去掉命令部分）
  const remainingText = normalizedText.replace(matchedPattern.toLowerCase(), '').trim();
 ...
 * 提取命令参数
 */
export function extractCommandParameters(text: string): Record<string, any> {
  ...
  } else if (text.includes('设定')) {
    parameters.type = 'setting';
  } else if (text.includes('章节')) {
    parameters.type = 'chapter';
  }
  
  // 提取位置参数（如"在开头插入"）
  if (text.includes('开头')) {
    parameters.position = 'start';
  } else if (text.includes('结尾')) {
    parameters.position = 'end';
  } else if (text.includes('中间')) {
