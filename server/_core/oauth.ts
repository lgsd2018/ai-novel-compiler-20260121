...
export function registerOAuthRoutes(app: Express) {
  ...
  console.log("[OAuth] Dev login check:", {
    NODE_ENV: process.env.NODE_ENV,
    ENABLE_DEV_LOGIN: process.env.ENABLE_DEV_LOGIN,
    enabled: enableDevLogin
  });

  if (enableDevLogin) {
    console.log("[OAuth] Registering /api/oauth/dev-login");
    app.get("/api/oauth/dev-login", async (req: Request, res: Response) => {
      console.log("[OAuth] /api/oauth/dev-login called");
      try {
        ...
        const cookieOptions = getSessionCookieOptions(req);
        console.log("[OAuth] Setting cookie:", COOKIE_NAME, "with options:", cookieOptions);
        res.cookie(COOKIE_NAME, sessionToken, { ...cookieOptions, maxAge: ONE_YEAR_MS });
